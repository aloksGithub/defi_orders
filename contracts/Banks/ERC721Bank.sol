...
contract ERC721Bank is BankBase {
    ...
    function setWrapper(address nftManager, address wrapper)
        external
        onlyOwner
    {
    ...
    function encodeId(uint256 id, address nftManager)
        public
        view
        returns (uint256)
    {
        ...
        )
    {
        ...
        poolAddress = IERC721Wrapper(erc721Wrappers[nftManager]).getPoolAddress(
                nftManager,
                pos_id
            );
    }
    ...
    function getLPToken(uint256 id)
        ...
        view
        override
        returns (address managerAddress)
    {
        (, managerAddress, ) = decodeId(id);
    }

    function getIdFromLpToken(address manager)
        public
        view
        override
    ...
    function mint(
        ...
        (, , , , , , , uint256 minted, , , , ) = INonfungiblePositionManager(
            suppliedTokens[0]
        ).positions(suppliedAmounts[0]);
    ...
    function mintRecurring(
        ...
        require(
            poolInfo[tokenId].user == userAddress,
            "9"
        );
        ...
        bytes memory returnData = wrapper.functionDelegateCall(
            abi.encodeWithSelector(
                IERC721Wrapper.deposit.selector,
                manager,
                id,
                suppliedTokens,
                suppliedAmounts
        ...
        returns (address[] memory outTokens, uint256[] memory tokenAmounts)
    {
        ...
        bytes memory returnData = wrapper.functionDelegateCall(
            abi.encodeWithSelector(
                IERC721Wrapper.withdraw.selector,
                manager,
                id,
                amount,
                receiver
        ...
        (outTokens, tokenAmounts) = abi.decode(
            returnData,
            (address[], uint256[])
        );
        ...
        )
    {
        ...
        bytes memory returnData = wrapper.functionDelegateCall(
            abi.encodeWithSelector(
                IERC721Wrapper.harvest.selector,
                manager,
                id,
                receiver
            )
        ...
        poolInfo[tokenId].liquidity = liquidity;
        (rewardAddresses, rewardAmounts) = abi.decode(
            returnData,
            (address[], uint256[])
        );
    ...
    function getUnderlyingForFirstDeposit(uint256 tokenId)
        public
        view
        override
    ...
    function getUnderlyingForRecurringDeposit(uint256 tokenId)
        public
        view
        override
        returns (address[] memory, uint256[] memory ratios)
    {
    ...
    function getRewards(uint256 tokenId)
        ...
        view
        override
        returns (address[] memory rewardsArray)
    {
