...
contract MasterChefBank is ERC1155("MasterChefBank"), BankBase {
    ...
    function encodeId(address masterChef, uint256 pid)
        public
        pure
        returns (uint256)
    {
    ...
    function setMasterChefWrapper(address masterChef, address wrapper)
        external
        onlyOwner
    {
        ...
        )
    {
        ...
        lpToken = IMasterChefWrapper(masterChefWrappers[masterChef]).getLpToken(
                pid
            );
    }

    function getLPToken(uint256 id)
        public
        view
        override
    ...
    {
        for (uint256 i = 0; i < supportedMasterChefs.length; i++) {
            IMasterChefWrapper wrapper = IMasterChefWrapper(
                masterChefWrappers[supportedMasterChefs[i]]
            );
        ...
        returns (address[] memory)
    {
        ...
        IMasterChefWrapper wrapper = IMasterChefWrapper(
            masterChefWrappers[masterChef]
        );
        return wrapper.getRewards(pid);
    }

    function getPendingRewardsForUser(uint256 tokenId, address user)
        external
        view
        override
        returns (address[] memory rewards, uint256[] memory amounts)
    {
        ...
        (rewards, everyonesRewardAmounts) = IMasterChefWrapper(
            masterChefWrappers[masterChef]
        ).getPendingRewards(pid);
        amounts = new uint256[](rewards.length);
        if (lpSupply > 0) {
            for (uint256 i = 0; i < rewards.length; i++) {
                address reward = rewards[i];
                uint256 allocationPerShare = pool.rewardAllocationsPerShare[
                    rewards[i]
                ] + (everyonesRewardAmounts[i] * PRECISION) / lpSupply;
                int256 accumulatedReward = int256(
                    (pool.userShares[user] * allocationPerShare) / PRECISION
                );
                uint256 pendingReward = uint256(
                    accumulatedReward - pool.rewardDebt[user][reward]
                );
    ...
    function getPositionTokens(uint256 tokenId, address userAddress)
        external
        view
        override
        returns (address[] memory outTokens, uint256[] memory tokenAmounts)
    {
    ...
    function updateToken(uint256 tokenId) internal onlyAuthorized {
        ...
        uint256 lpSupply = pool.lpSupply;
        if (lpSupply > 0) {
            address[] memory rewards = IMasterChefWrapper(
                masterChefWrappers[masterChef]
            ).getRewards(pid);
            ...
            for (uint256 i = 0; i < rewards.length; i++) {
                rewardAmounts[i] =
                    ...
                    rewardAmounts[i];
                pool.rewardAllocationsPerShare[rewards[i]] +=
                    (rewardAmounts[i] * PRECISION) /
                    lpSupply;
            }
        }
    }
    ...
    function _deposit(
        ...
        masterChefWrapper.functionDelegateCall(
            abi.encodeWithSelector(
                IMasterChefWrapper.deposit.selector,
                masterChef,
                pid,
                amount
            )
    ...
    function _withdraw(
        ...
        masterChefWrapper.functionDelegateCall(
            abi.encodeWithSelector(
                IMasterChefWrapper.withdraw.selector,
                masterChef,
                pid,
                amount
            )
    ...
    function mint(
        ...
        for (uint256 i = 0; i < rewards.length; i++) {
            ...
            pool.rewardDebt[userAddress][reward] += int256(
                (suppliedAmounts[0] * pool.rewardAllocationsPerShare[reward]) /
                    PRECISION
            );
        }
        ...
        )
    {
        ...
        for (uint256 i = 0; i < rewards.length; i++) {
            address reward = rewards[i];
            int256 accumulatedReward = int256(
                (pool.userShares[userAddress] *
                    pool.rewardAllocationsPerShare[reward]) / PRECISION
            );
            uint256 pendingReward = uint256(
                accumulatedReward - pool.rewardDebt[userAddress][reward]
            );
