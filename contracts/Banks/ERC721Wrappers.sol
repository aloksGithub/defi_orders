...
abstract contract IERC721Wrapper is Ownable {
    function isSupported(address manager, address pool)
        external
        view
        virtual
        returns (bool);

    function getPoolAddress(address manager, uint256 id)
        external
        view
        virtual
        returns (address);
    ...
    function getRatio(address manager, uint256 id)
        external
        view
        virtual
        returns (address[] memory tokens, uint256[] memory ratios);

    function getRewardsForPosition(address manager, uint256 tokenId)
        external
        view
        virtual
        returns (address[] memory rewards, uint256[] memory amounts);

    function getERC20Base(address pool)
        external
        view
        virtual
        returns (address[] memory underlyingTokens);

    function getPositionUnderlying(address manager, uint256 tokenId)
        external
        view
        virtual
...
contract UniswapV3Wrapper is IERC721Wrapper {
    ...
    function isSupported(address managerAddress, address poolAddress)
        external
        view
        override
        returns (bool)
        ...
        returns (address)
    {
        ...
        ) = INonfungiblePositionManager(manager).positions(id);
    ...
    function deposit(
        ...
        for (uint256 i = 0; i < suppliedTokens.length; i++) {
            uint256 allowance = IERC20(suppliedTokens[i]).allowance(
                address(this),
                manager
            );
            IERC20(suppliedTokens[i]).safeDecreaseAllowance(manager, allowance);
            IERC20(suppliedTokens[i]).safeIncreaseAllowance(
                manager,
                suppliedAmounts[i]
            );
        }
        ...
        (uint256 minted, uint256 a0, uint256 a1) = INonfungiblePositionManager(
            manager
    ...
    function getRatio(address manager, uint256 tokenId)
        external
        view
        override
        returns (address[] memory tokens, uint256[] memory ratios)
    {
        ...
        IUniswapV3Factory factory = IUniswapV3Factory(
            INonfungiblePositionManager(manager).factory()
        );
        ...
        returns (address[] memory tokens, uint256[] memory amounts)
    {
        ...
        IUniswapV3Pool pool = IUniswapV3Pool(
            IUniswapV3Factory(INonfungiblePositionManager(manager).factory())
    ...
    function getERC20Base(address poolAddress)
        external
        view
        override
        returns (address[] memory underlyingTokens)
    ...
    function getPendingFees(address manager, uint256 _positionId)
        internal
        view
        returns (uint256 feeAmt0, uint256 feeAmt1)
    {
        ...
        (uint128 liquidity, , , uint128 tokensOwed0, uint128 tokensOwed1) = pool
            .positions(_getPositionID(manager, tickLower, tickUpper));
